...
export type OptqRequestHeaders<
  Api,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> = never,
> = Equals<ApiId, never> extends true
  ? "requestHeaders" extends keyof Api
    ? NormalizeHeader<Api["requestHeaders"]>
    : EmptyObject
  : "requestHeaders" extends keyof Api[ApiId]
    ? OptqRequestHeaders<Api> & NormalizeHeader<Api[ApiId]["requestHeaders"]>
    : OptqRequestHeaders<Api>;
export type OptqResponseHeaders<
  Api,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> = never,
> = Equals<ApiId, never> extends true
  ? "responseHeaders" extends keyof Api
    ? NormalizeHeader<Api["responseHeaders"]>
    : EmptyObject
  : "responseHeaders" extends keyof Api[ApiId]
    ? OptqResponseHeaders<Api> & NormalizeHeader<Api[ApiId]["responseHeaders"]>
    : OptqResponseHeaders<Api>;
export type OptqParams<
  Api extends OptqApiBase<Api>,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> = never,
> = Equals<ApiId, never> extends true
  ? "params" extends keyof Api
    ? Api["params"]
    : EmptyObject
  : "params" extends keyof Api[ApiId]
    ? OptqParams<Api> & Api[ApiId]["params"]
    : OptqParams<Api>;
export type OptqRequestBody<
  Api,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys>,
> = ApiId extends `GET ${string}`
  ? never
  : "body" extends keyof Api[ApiId]
    ? Api[ApiId]["body"]
    : undefined;
export type OptqResourceData<
  Api extends OptqApiBase<Api>,
  Path extends string,
> = `GET ${Path}` extends keyof Api
  ? "data" extends keyof Api[`GET ${Path}`]
    ? Api[`GET ${Path}`]["data"]
    : never
  : never;
export type OptqResourceId<Api extends OptqApiBase<Api>> = OptqResourceIdDistribute<
  Exclude<keyof Api, OptqAdditionalApiTypeKeys> & string
>;
...
export type OptqApiBase<Api> = keyof Api extends OptqAllowedApiTypeKeys
  ? {
      ...
      [K in keyof Api as K extends `${Method} ${string}` ? K : never]: K extends `${infer M extends
        Method} ${infer P extends string}`
        ? PrettifyOptional<{
            params: Equals<ExtractPathParams<P>, never> extends true
            ...
            data: M extends "GET"
              ? Exclude<
                  "data" extends keyof Api[K] ? Api[K]["data"] : OptqErrorDataMissingInGetRoute<K>,
                  undefined
                >
              : unknown;
            error?: {
...
export type OptqConfig<Api extends OptqApiBase<Api>> = {
  ...
  routes?: {
    [K in Exclude<keyof Api, OptqAdditionalApiTypeKeys>]?: K extends `${infer M extends
      Method} ${infer P extends string}` &
      keyof Api
      ? M extends "GET"
        ? OptqResourceRouteConfig<Api, K & `GET ${P}`, P>
        : M extends Exclude<Method, "GET">
          ? OptqMutationRouteConfig<Api, M, K>
          : never
      : never;
  };
};

export type OptqResourceRouteConfig<
  Api extends OptqApiBase<Api>,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> & `GET ${string}`,
  Path extends string,
> = {
  respondedAt?(response: OptqResponse<Api, ApiId>): bigint | number;
  hash?(params: OptqParams<Api, ApiId>): string;
  defaultValue?:
    | OptqResourceData<Api, Path>
    | ((params: OptqParams<Api, ApiId>) => OptqResourceData<Api, Path>);
};

export type OptqMutationRouteConfig<
  Api extends OptqApiBase<Api>,
  M extends Exclude<Method, "GET">,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> & `${M} ${string}`,
> = {
  respondedAt?(response: OptqResponse<Api, ApiId>): bigint | number;
  actions?(
    requestWithSet: OptqRequest<Api, ApiId> & {
      set<ResId extends OptqResourceId<Api>>(
        resourceId: ResId,
        params: OptqParams<Api, `GET ${ResId}` & Exclude<keyof Api, OptqAdditionalApiTypeKeys>>,
        update:
          | (OptqResourceData<Api, ResId> | undefined)
          | ((
              prev: OptqResourceData<Api, ResId> | undefined,
            ) => OptqResourceData<Api, ResId> | undefined),
      ): void;
    },
  ): void;
  onResponse?(
    response: OptqMutationResponse<Api, M, ApiId> & {
      ...
      set<ResId extends OptqResourceId<Api>>(
        resourceId: ResId,
        params: OptqParams<Api, `GET ${ResId}` & Exclude<keyof Api, OptqAdditionalApiTypeKeys>>,
        data: OptqResourceData<Api, ResId>,
      ): void;
      removeRequest(): void;
    },
  ): void;
...
export type OptqResponse<
  Api extends OptqApiBase<Api>,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> = never,
> = Equals<ApiId, never> extends true
  ? { headers: OptqResponseHeaders<Api> }
  : ApiId extends `GET ${string}`
    ? OptqGetResponse<Api, ApiId>
    : ApiId extends `${infer M extends Exclude<Method, "GET">} ${string}`
      ? OptqMutationResponse<Api, M, ApiId>
      : never;

export type OptqGetResponse<
  Api extends OptqApiBase<Api>,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> & `GET ${string}`,
> = "data" extends keyof Api[ApiId]
  ? Prettify<
      | {
          status: OkStatus;
          ok: true;
      ...
      | ({
          headers: OptqResponseHeaders<Api>;
        } & ("error" extends keyof Api[ApiId]
          ? Api[ApiId]["error"] extends {
              status: ErrorStatus;
              headers?: AnyHeaders;
              data?: unknown;
            }
...
export type OptqMutationResponse<
  Api extends OptqApiBase<Api>,
  M extends Exclude<Method, "GET">,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> & `${M} ${string}`,
> = Prettify<
  ...
  | ("error" extends keyof Api[ApiId]
      ? Api[ApiId]["error"] extends {
        ...
        ? OptqMutationResponseDistributeError<Api, Api[ApiId]["error"]>
        : never
      : {
          status: ErrorStatus;
          ok: false;
...
type OptqMutationResponseDistributeError<
  Api,
  E extends {
    status: ErrorStatus;
    headers?: AnyHeaders;
...
> = E extends unknown
  ? {
      ...
      headers: "headers" extends keyof E
        ? Prettify<OptqResponseHeaders<Api> & E["headers"]>
        : OptqResponseHeaders<Api> & AnyHeaders;
      data: E["data"];
    }
  : never;

export type OptqRequestStore<Api extends OptqApiBase<Api>> = OptqRequestStoreDistribute<
  Api,
  Exclude<keyof Api, OptqAdditionalApiTypeKeys> & `${Exclude<Method, "GET">} ${string}`
>[];
type OptqRequestStoreDistribute<
  Api extends OptqApiBase<Api>,
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> &
    `${Exclude<Method, "GET">} ${string}`,
> = ApiId extends unknown
  ? Prettify<
      OptqRequest<Api, ApiId> & {
        respondedAt?: number | bigint;
...
export type OptqSetter<Api extends OptqApiBase<Api>> = <ResId extends OptqResourceId<Api>>(
...
> extends true
  ? [params?: OptqParams<Api, `GET ${ResId}` & Exclude<keyof Api, OptqAdditionalApiTypeKeys>>]
  : [params: OptqParams<Api, `GET ${ResId}` & Exclude<keyof Api, OptqAdditionalApiTypeKeys>>];
export type OptqGetter<Api extends OptqApiBase<Api>> = <ResId extends OptqResourceId<Api>>(
  resourceId: ResId,
  ...optionalParams: OptqGetterOptionalParams<Api, ResId>
) => OptqResourceData<Api, ResId> | undefined;

export type OptqMutator<Api extends OptqApiBase<Api>> = <
  ApiId extends Exclude<keyof Api, OptqAdditionalApiTypeKeys> &
    `${Exclude<Method, "GET">} ${string}`,
>(
  req: Omit<OptqRequest<Api, ApiId>, "id"> & { id?: string },
) => Promise<
  OptqResponse<Api, ApiId> & {
    status: number;
    ok: boolean;
    data?: unknown;
  }
